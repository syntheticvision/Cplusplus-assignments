...
namespace seneca
{
	...
	int ProcessData::operator()(const std::string &target_file, double &avg, double &var)
	{
		if (!*this)
		{
			throw std::string("Object is not in a valid state.");
		}
		...
		for (int i = 0; i < num_threads; ++i)
		{
			threads.push_back(std::thread(computeVarFactor, data + p_indices[i], p_indices[i + 1] - p_indices[i], total_items, avg, std::ref(variances[i])));
		}
		...
		if (out_file)
		{
			out_file.write(reinterpret_cast<char *>(&total_items), sizeof(total_items));
			out_file.write(reinterpret_cast<char *>(data), total_items * sizeof(int));
			out_file.close();
		}
		else
		{
			throw std::string("Error: could not open file " + target_file);
		}
